...
- `GET /v1/product-hunt/leaderboard` — Fetch a ranked daily, weekly, or monthly leaderboard, optionally filtered by topic. Fast, but always partial (`truncated: true`, only the first ~19-35 products).
- `GET /v1/product-hunt/leaderboard/crawl` — Queue an async job that scrolls the leaderboard to its genuine end (`truncated: false`). Poll via [Jobs](/api-reference/jobs).
