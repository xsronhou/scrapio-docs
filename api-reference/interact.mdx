...
For targets that block non-residential traffic, add `"proxy": "stealth"` (or the deprecated `"use_proxy": true`) to launch the session through a residential proxy instead of the default direct connection. Requires a paid plan (Starter or higher) and adds a 65-credit surcharge on top of the base 25 (90 total) — see [Rate Limits & Credits](/rate-limits).
