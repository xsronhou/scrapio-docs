...
<Tabs>
  <Tab title="Claude Desktop">
    ...
    <Note>
      Prefer not to hand-edit JSON? Claude Desktop's **Settings → Connectors → Add custom connector** accepts a URL and handles login for you — enter `https://mcp.scrapio.dev/mcp` (the hosted endpoint, see the Hosted tab below) and you'll be sent through a normal login-and-approve screen, no API key to paste anywhere. It's built for the HTTP transport, not for spawning a local `npx` process, so this only works with the hosted endpoint or your own server running in HTTP mode.
    </Note>
  </Tab>
