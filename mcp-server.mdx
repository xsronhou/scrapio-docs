...
<Tabs>
  <Tab title="Claude Desktop">
    ...
    <Note>
      Prefer not to hand-edit JSON? Claude Desktop's **Settings → Connectors → Add custom connector** lets you add a server by pasting a URL instead — but it's built for the HTTP transport, not for spawning a local `npx` process. Run the server once in HTTP mode (see the HTTP tab below), then enter `http://localhost:3010/mcp` in that UI.
    </Note>
  </Tab>
  <Tab title="Cursor">
