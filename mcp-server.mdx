...
The Scrapio MCP server gives AI agents direct access to the full web data surface through a single registered server — fetch, crawl, Google and Fast Search, YouTube, Amazon and Walmart product data, Booking.com and Agoda hotel data, browser automation, and async jobs.
...
<Tabs>
  <Tab title="Claude Desktop">
    ...
    <Note>
      Prefer not to hand-edit JSON? Claude Desktop's **Settings → Connectors → Add custom connector** lets you add a server by pasting a URL instead — but it's built for the HTTP transport, not for spawning a local `npx` process. Enter `https://mcp.scrapio.dev/mcp` to use the hosted endpoint (see the Hosted tab below), or run the server yourself in HTTP mode and enter `http://localhost:3010/mcp`.
    </Note>
  </Tab>
  <Tab title="Hosted (no install)">
    ...
    - **Auth:** send `Authorization: Bearer <SCRAPIO_API_KEY>` on every request (most MCP clients that support remote/HTTP servers have a field for this — for the JSON-based configs above, add a `"headers"` block instead of `"env"`):
    ...
    Each API key gets its own isolated session — the hosted endpoint is multi-tenant, so different keys never see each other's sessions or data. This is the same underlying server as the self-hosted HTTP mode below, just running with per-request auth instead of one fixed key.
  </Tab>
  <Tab title="Cursor">
...
| `fast_search` | Lightweight organic + news search, priced below `google_search` |
...
| `youtube_search` | Search YouTube and return structured video results |
| `youtube_video` | Get structured metadata for a single YouTube video |
| `youtube_search_crawl` | Queue an async, multi-page YouTube search crawl — returns a job ID |
